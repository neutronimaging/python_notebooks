...
class TestCombineImagesWithOutliners:
	...
	def test_make_sure_arrays_have_the_same_size(self):
		...
		with pytest.raises(ValueError):
			CombineImages.check_arrays_have_same_size_and_dimensions(list_array=list_array)
	...
	def test_mean_without_outliers_3_arrays(self):
		...
		mean_array_calculated = CombineImages.mean_without_outliers(list_array=[array1, array2, array3])
	...
	def test_mean_without_outliers_4_arrays(self):
		...
		mean_array_calculated = CombineImages.mean_without_outliers(list_array=[array1, array2, array3, array4])
