...
class ExtractEvenlySpacedFiles:
    working_dir = ""
    ...
    def select_folder(self):
        self.folder_widget = FileSelectorPanel(
    ...
    def renamed_files(self):
        ...
        self.left_vertical_layout = widgets.VBox(
            [
                widgets.Label("Before renaming", layout=widgets.Layout(width="100%")),
                widgets.Select(
    ...
    def select_output_folder(self):
        self.output_folder_ui = FileSelectorPanelWithJumpFolders(
            instruction="select where to extract the files",
            start_dir=self.working_dir,
            next=self.extract,
            type="directory",
    ...
    def extract(self, output_folder):
        ...
        if self.renaming_files_widget.value:
            ...
            display(
                HTML(
                    '<span style="font-size: 15px; color:green">Copying and renaming '
            ...
            file_handler.copy_and_rename_files_to_folder(
            ...
            display(
                HTML(
                    '<span style="font-size: 15px; color:blue">'
                    + str(len(list_of_files_to_extract))
                    + " files have been copied and renamed into "
                    + full_output_folder_name
                    + "... DONE!</span>"
                )
            )
            ...
            display(
                HTML(
                    '<span style="font-size: 15px; color:green">Copying '
                    + str(len(list_of_files_to_extract))
                    + " files into "
                    + full_output_folder_name
                    + "... IN PROGRESS!</span>"
