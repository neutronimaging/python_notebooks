...
class ExportCalibration:
    ...
    def get_metadata(self):
        ...
        if self.parent.ui.use_calibration1_checkbox.isChecked():
            ...
            metadata.append(f"#   x0: {self.parent.ui.calibration1_x0.text()!s}")
            metadata.append(f"#   y0: {self.parent.ui.calibration1_y0.text()!s}")
            metadata.append(f"#   width: {self.parent.ui.calibration1_width.text()!s}")
            metadata.append(
                f"#   height: {self.parent.ui.calibration1_height.text()!s}"
            )
            metadata.append(
                f"#   file index: {self.parent.ui.calibration1_index.text()!s}"
            )
            metadata.append(
                f"#   value requested: {self.parent.ui.calibration1_value.text()!s}"
            )
        if self.parent.ui.use_calibration2_checkbox.isChecked():
            metadata.append("#Calibration Region 2:")
            metadata.append(f"#   x0: {self.parent.ui.calibration2_x0.text()!s}")
            metadata.append(f"#   y0: {self.parent.ui.calibration2_y0.text()!s}")
            metadata.append(f"#   width: {self.parent.ui.calibration2_width.text()!s}")
            metadata.append(
                f"#   height: {self.parent.ui.calibration2_height.text()!s}"
            )
            metadata.append(
                f"#   file index: {self.parent.ui.calibration2_index.text()!s}"
            )
            metadata.append(
                f"#   value requested: {self.parent.ui.calibration2_value.text()!s}"
            )
