...
from __code.group_images_by_cycle_for_grating_experiment.excel_handler import (
    ExcelHandler,
)
...
class EventHandler:
    ...
    def group_index_changed(self, value):
        ...
        short_list_new_files = o_get.list_of_new_files_basename_only(
            int(new_group_index)
        )
    ...
    def use_excel_file_clicked(self, state):
        ...
        o_file = FileFolderBrowser(
            working_dir=self.parent.output_folder, next_function=self.load_excel_file
        )
        o_file.select_images(
            instruction="Select Excel file ...",
            multiple_flag=False,
            filters={"excel": "*.xls"},
