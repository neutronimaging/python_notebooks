...
class NotebookWidgets:
    ...
    def select_metadata_to_use_for_sorting(self):
        ...
        # metadata_outer
        search_outer = widgets.HBox(
            [
                widgets.Label("Search:"),
                widgets.Text("", layout=widgets.Layout(width="150px")),
                widgets.Button(description="X", button_style="", layout=widgets.Layout(width="10px")),
            ]
        )
        ...
        result2 = widgets.HBox(
            [
                widgets.HTML("<u>Metadata Selected</u>:", layout=widgets.Layout(width="200px")),
                widgets.Label(metadata_outer_value, layout=widgets.Layout(width="100%")),
            ]
        )
        self.parent.metadata_outer_selected_label = result2.children[1]

        metadata_outer = widgets.VBox(
            [
                widgets.HTML("<b>Outer Loop Metadata</b>"),
                search_outer,
                widgets.Select(
                    options=list_metadata,
                    value=metadata_outer_value,
                    layout=widgets.Layout(width=select_width, height=select_height),
                ),
                result2,
            ]
        )
        ...
        result1 = widgets.HBox(
            [
                widgets.HTML("<u>Metadata Selected</u>:", layout=widgets.Layout(width="200px")),
                widgets.Label(metadata_inner_value, layout=widgets.Layout(width="100%")),
            ]
        )
        self.parent.metadata_inner_selected_label = result1.children[1]

        metadata_inner = widgets.VBox(
            [
                widgets.HTML("<b>Inner Loop Metadata</b>"),
                search_inner,
                widgets.Select(
                    options=list_metadata,
        ...
        switch_button = widgets.Button(description="Switch", icon="refresh", layout=widgets.Layout(display="center"))
        switch_layout = widgets.VBox(
            [switch_button], layout=widgets.Layout(height="400px", display="center", justify_content="space-around")
        )
    ...
    def display_groups(self):
        ...
        vbox_left = widgets.VBox(
            [
                widgets.HTML("<b>Select Group</b>:"),
                widgets.Select(options=group_label, layout=widgets.Layout(width="150px", height="300px")),
            ]
        )
        ...
        vbox_center = widgets.VBox(
            [
                widgets.HTML("<b>Original file names</b>:"),
                widgets.Select(
                    options=o_get.list_of_files_basename_only(0), layout=widgets.Layout(width="450px", height="300px")
                ),
            ]
        )
        ...
        vbox_3 = widgets.VBox(
            [
                ...
                widgets.Select(
                    options=o_get.list_of_new_files_basename_only(0),
        ...
        vbox_right = widgets.VBox(
            [
                ...
                widgets.Textarea(value="", layout=widgets.Layout(width="200px", height="300px")),
            ]
        )
        ...
        message = widgets.HTML(
            "<b><font color='blue'>INFO</font></b>: <i>if more than 1 image are in the same "
        ...
        bottom_hbox = widgets.HBox(
            [
                widgets.HTML("<b>Images are in</b>:", layout=widgets.Layout(width="150px")),
                widgets.Label(self.parent.data_path, layout=widgets.Layout(width="90%")),
            ]
        )
    ...
    def generate_angel_configuration_file(self):
        ...
        load_previous_excel_ui = widgets.Button(
            ...
            icon="edit",
            layout=widgets.Layout(width="50%", border="2px solid green"),
        )
        ...
        hori_box = widgets.HBox([load_previous_excel_ui, create_new_excel_ui], layout=hori_layout)
        display(hori_box)
    ...
    def define_type_of_data(self):
        sample_or_ob_ui = widgets.HBox(
            [
                widgets.HTML("Which type of data are you working with?", layout=widgets.Layout(width="250px")),
                widgets.RadioButtons(options=["sample", "ob"], value="sample", layout=widgets.Layout(width="50%")),
                widgets.HTML("<hr>"),
            ]
        )
