...
class CombineImagesNByN:
    ...
    def how_many_files(self):
        ...
        vertical_layout = widgets.VBox(
            [
                widgets.Dropdown(options=radio_list_string, value=radio_list_string[0]),
                widgets.Label("", layout=widgets.Layout(width="100%")),
            ]
        )
    ...
    def update_how_many_files(self, bin_value_object=None):
        ...
        message = f"You are about to create {nbr_images} files out of {len(self.list_files_sorted)} files selected."
    ...
    def create_dictionary_of_new_file_names(self):
        ...
        if renaming_option == RENAMING_OPTIONS[0]:
            ...
            for _key in dict_list_files.keys():
                new_file_name = f"image_{_key:04d}.tiff"
    ...
    def merging(self, output_folder):
        ...
        CombineImagesNByN.combine_timespectra(
            input_timespectra_file_name=self.timespectra_file_name,
        ...
        if self.timespectra_file_name:
            display(
                HTML(
                    '<span style="font-size: 20px; color:blue"> A new _Spectra.txt file has been created: '
                    + output_timespectra_file_name
                    + "</span>"
                )
            )
    ...
    def __create_merged_file_name(index=0):
        ...
        return FILE_PREFIX + f"_{index:03d}.tiff"
