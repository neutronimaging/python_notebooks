...
from __code.group_images_by_cycle_for_panoramic_stitching.group_images_by_cycle import (
    GroupImagesByCycle,
)
from __code.group_images_by_cycle_for_panoramic_stitching.sort_images_within_each_cycle import (
...
class GroupImages:
    ...
    how_to_sort_within_cycle = {
        "1st_variable": {"name": "", "is_ascending": True},
        "2nd_variable": {"name": "", "is_ascending": True},
    }
    ...
    def select_input_folder(self):
        self.files_list_widget = fileselector.FileSelectorPanel(
            instruction="select folder of images to sort",
    ...
    def group_images(self):
        ...
        dict_new_names = GroupImages.make_dictionary_of_groups_new_names(
            self.dictionary_of_groups_unsorted
        )
    ...
    def make_dictionary_of_groups_new_names(dictionary_of_group_unsorted):
        ...
        for _group_index in dictionary_of_group_unsorted.keys():
            ...
            list_new_names = [
                NEW_FILE_NAME_PREFIX + f"{_index:04d}.tiff"
                for _index in np.arange(general_index, general_index + nbr_files)
            ]
    ...
    def display_groups(self):
        ...
        group_label = [f"Group # {_index}" for _index in np.arange(nbr_groups)]
        self.list_group_label = group_label
        vbox_left = widgets.VBox(
            [
                widgets.Label("Select Group:"),
                widgets.Select(
                    options=group_label,
                    layout=widgets.Layout(width="100px", height="300px"),
                ),
            ]
        )
        select_group_ui = vbox_left.children[1]
        select_group_ui.observe(self.group_index_changed, "value")
        vbox_center = widgets.VBox(
            [
                widgets.Label("Original file names:"),
                widgets.Select(
                    options=self.get_list_of_files_basename_only(0),
                    layout=widgets.Layout(width="450px", height="300px"),
                ),
            ]
        )
        ...
        vbox_right = widgets.VBox(
            [
                ...
                widgets.Textarea(
                    value="", layout=widgets.Layout(width="200px", height="300px")
                ),
            ]
        )
        self.metadata_ui = vbox_right.children[1]
        self.list_of_files_changed(
            value={"new": self.get_list_of_files_basename_only(0)[0]}
        )
        ...
        bottom_hbox = widgets.HBox(
            [
                widgets.Label("Images are in:"),
                widgets.Label(self.data_path, layout=widgets.Layout(width="90%")),
            ]
        )
    ...
    def list_of_files_changed(self, value):
        ...
        string_to_display = ""
        for _key, _value in dictionary_file_vs_metadata[
            full_file_name_selected
        ].items():
            string_to_display += f"{_key}: {_value}\n"
    ...
    def how_to_sort_files(self):
        ...
        vbox1 = widgets.VBox(
            [
                widgets.Label("Metadata name:", layout=widgets.Layout(width="150px")),
                widgets.Select(
                    options=self.metadata_name_to_select,
                    value=self.metadata_name_to_select[0],
                    layout=widgets.Layout(width="400px", height="50px"),
                ),
                widgets.RadioButtons(
                    options=["Ascending", "Descending"], value="Ascending"
                ),
            ]
        )
        ...
        self.how_to_sort_within_cycle["1st_variable"] = {
            "name": name_of_first_metadata_ui.value,
        ...
        name_of_second_metadata_ui.observe(
            self.name_of_second_metadata_changed, "value"
        )
        ...
        self.how_to_sort_within_cycle["2nd_variable"] = {
            "name": name_of_second_metadata_ui.value,
            "is_ascending": is_ascending,
        }

        self.how_to_sort_within_cycle["2nd_variable"]["name"] = (
            name_of_second_metadata_ui.value
        )
        ...
        hori1 = widgets.HBox(
            [
                widgets.Label("Select Group", layout=widgets.Layout(width="100px")),
                widgets.Dropdown(
                    options=self.list_group_label, layout=widgets.Layout(width="150px")
                ),
            ]
        )
        ...
        vbox3 = widgets.VBox(
            [
                widgets.Label("Old name", layout=widgets.Layout(width="200px")),
                widgets.Select(
                    options="", layout=widgets.Layout(width="400px", height="300px")
                ),
            ]
        )
        ...
        vbox4 = widgets.VBox(
            [
                widgets.Label("New name", layout=widgets.Layout(width="200px")),
                widgets.Select(
                    options="", layout=widgets.Layout(width="400px", height="300px")
                ),
            ]
    ...
    def update_old_name_order(self):
        ...
        o_sort = SortImagesWithinEachCycle(
            dict_groups_filename=self.dictionary_of_groups_unsorted,
        ...
        short_list_files_sorted = [
            os.path.basename(_file) for _file in list_files_sorted
        ]
    ...
    def select_output_folder(self):
        output_folder_widget = fileselector.FileSelectorPanel(
    ...
    def create_folder_for_each_cycle(self, output_folder):
        ...
        output_folder_basename = (
            os.path.basename(self.folder_selected) + "_sorted_by_cycle"
        )
        ...
        hbox = widgets.HBox(
            [
                widgets.IntProgress(value=0, min=0, max=nbr_groups),
                widgets.Label(f"0/{nbr_groups}"),
            ]
        )
        ...
        for _group_index in dictionary_of_groups_sorted.keys():
            full_folder_name = os.path.join(output_folder, f"group#{_group_index:02d}")
            make_or_reset_folder(full_folder_name)
            copy_and_rename_files_to_folder(
                list_files=dictionary_of_groups_sorted[_group_index],
        ...
        display(
            format_html_message(
                pre_message=message, spacer=" in ", message=output_folder
            )
        )
