...
class RegistrationMarkers(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        ui_full_path = os.path.join(
            ...
            os.path.join("ui", "ui_registration_markers.ui"),
        )
    ...
    def table_right_click(self, position):
        ...
        if left_column_selected in [1, 2]:
            ...
            if self.parent.marker_table_buffer_cell is not None:
        ...
        if action == copy_cell:
            ...
            self.paste_cell(
                top_row_selected=top_row_selected,
    ...
    def end_marker_initialized(self):
        ...
        o_table.set_item_with_str(row=starting_row_selected, column=3, cell_str="")

        xoffset_from = int(o_table.get_item_str_from_cell(row=from_row, column=1))
        yoffset_from = int(o_table.get_item_str_from_cell(row=from_row, column=2))

        xoffset_to = int(o_table.get_item_str_from_cell(row=to_row, column=1))
        yoffset_to = int(o_table.get_item_str_from_cell(row=to_row, column=2))
    ...
    def closeEvent(self, c):
        ...
        self.parent.set_widget_status(
            list_ui=[
                self.parent.ui.auto_registration_button,
