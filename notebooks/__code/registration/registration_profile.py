...
class RegistrationProfileUi(QMainWindow):
    ...
    def __init__(self, parent=None, data_dict=None):
        ...
        ui_full_path = os.path.join(
            ...
            os.path.join("ui", "ui_registration_profile.ui"),
        )
    ...
    def recalculate_current_peak(self, file_index=-1, is_horizontal=True):
        try:
            self.calculate_peak(file_index=file_index, is_horizontal=is_horizontal)
        except ValueError:
            return
    ...
    def display_current_peak(self, is_horizontal=True):
        ...
        if infinite_line_ui is not None:
            profile_2d_ui.removeItem(infinite_line_ui)

        infinite_line_ui = pg.InfiniteLine(angle=90, movable=False, pos=peak)
        profile_2d_ui.addItem(infinite_line_ui, ignoreBounds=True)
...
class Settings(QMainWindow):
    def __init__(self, parent=None):
        ...
        ui_full_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
            os.path.join("ui", "ui_registration_profile_settings.ui"),
        )
        self.ui = load_ui(ui_full_path, baseinstance=self)
        self.init_widgets()
