...
from .get import Get
from .utilities import linear_operation, string_cleaning
...
class DisplayMetadataPyqtUi:
    def __init__(self, parent=None):
        ...
        self.list_ui = {
            1: {
                "font_size_slider": self.parent.ui.font_size_slider,
            ...
            2: {
                "font_size_slider": self.parent.ui.font_size_slider_2,
        ...
        if self.parent.ui.enable_graph_checkbox.isChecked():
            ...
            if not self.is_axis_plotable(x_axis):
                self.parent.ui.statusbar.showMessage(
                    "Error Displaying Metadata Graph (x-axis has wrong format)!", 10000
                )
            ...
            if not self.is_axis_plotable(y_axis):
                self.parent.ui.statusbar.showMessage(
                    "Error Displaying Metadata Graph (y-axis has wrong format)!", 10000
                )
                self.parent.ui.statusbar.setStyleSheet("color: red")
                return
            ...
            if y_unit:
                y_axis_label = f'<html><font color="{color}" size="{graph_font_size}">{y_name} ({y_unit})</font></html>'
            else:
                y_axis_label = f'<html><font color="{color}" size="{graph_font_size}">{y_name}</font></html>'
            ...
            graph.plot(
                x=[clean_and_format_x_axis[current_index]],
