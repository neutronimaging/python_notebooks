...
class ExportImagesAndVideo:
    ...
    def load_tiff_images_to_stack(self, image_files, output_stack_path=None):
        ...
        for i, img_path in enumerate(image_files[1:], 1):
            print(
                f"Loading image {i + 1}/{len(image_files)}: {os.path.basename(img_path)}"
            )
    ...
    def create_avi_from_tiff_stack(
        self,
        tiff_stack_path,
        output_avi_path=None,
        fps=10,
        codec="MJPG",
        ...
        try:
            ...
            writer = imageio.get_writer(
                output_avi_path, format="FFMPEG", mode="I", fps=fps, **codec_params
            )
            ...
            for i in range(frame_count):
                ...
                processed_frame = self._process_frame_for_avi(
                    frame, preserve_grayscale, codec
                )
            ...
            print(f"Error creating AVI video: {e!s}")
            return None
    ...
    def create_mp4_from_tiff_stack(self, tiff_stack_path, output_mp4_path=None, fps=20):
        ...
        try:
            ...
            print(f"Error creating MP4: {e!s}")
            return None

    def export_avi_like_imagej(
        self, tiff_stack_path=None, codec="MJPG", fps=10, quality="High"
    ):
        """
        ...
        if avi_path:
            ...
            display(
                HTML(
                    f"<b>ImageJ-style AVI created:</b> {avi_path} ({file_size:.1f} MB)"
                )
            )
