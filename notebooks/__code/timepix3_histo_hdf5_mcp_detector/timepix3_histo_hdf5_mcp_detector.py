...
import numpy as np
from __code._utilities import ANGSTROMS, LAMBDA, MICRO
...
from __code.timepix3_histo_hdf5_mcp_detector import (
    LIST_ELEMENTS,
    LIST_ELEMENTS_SUPPORTED,
    DefaultFittingParameters,
    FittingRegions,
    JSONKeys,
)
...
class Timepix3HistoHdf5McpDetector:
    ...
    default_parameters = {
        ...
        JSONKeys.fitting_parameters: {
            JSONKeys.a0: DefaultFittingParameters.a0,
    ...
    def select_nexus(self):
        self.nexus_ui = fileselector.FileSelectorPanel(
            instruction="Select Histo HDF5 file ...",
    ...
    def select_config(self):
        self.config_ui = fileselector.FileSelectorPanel(
    ...
    def load_config(self, config_file_name):
        ...
        display(
            HTML(
                '<span>Config file loaded: </span><span style="font-size: 20px; color:blue">'
                + config_file_name
                + "</span>"
            )
        )
    ...
    def calculate_and_display_profile(self):
        ...
        fig1, ax1 = plt.subplots(figsize=(8, 8), nrows=1, ncols=1)
        ...
        fig2, ax2 = plt.subplots(figsize=(8, 8), nrows=1, ncols=1)
        ...
        def plot_profile(x_axis, dSD_m, offset_micros, time_shift, element):
            ...
            if x_axis == "TOF":
                ...
                _exp = Experiment(
                    tof=tof_array * 1e-6,  # to convert to seconds
            ...
            ax2.plot(x_axis_array, _profile_shifted, color=list_matplotlib_colors[0])
        ...
        self.v = interactive(
            ...
            dSD_m=widgets.FloatSlider(
                value=self.default_parameters[JSONKeys.dSD_m],
                min=15,
                max=25,
                step=0.001,
    ...
    def select_peak_to_fit(self):
        ...
        list_matplotlib_colors = Color.list_matplotlib
        fig3, ax3 = plt.subplots(figsize=(8, 8), nrows=1, ncols=1)
        ...
        def plot_peaks(left_range, right_range, left_edge, right_edge):
            ...
            ax3.plot(lambda_x_axis, _profile, color=list_matplotlib_colors[0])
        ...
        self.peak_to_fit = interactive(
            plot_peaks,
            left_range=widgets.FloatSlider(
                min=np.min(lambda_x_axis),
            ...
            right_range=widgets.FloatSlider(
                min=np.min(lambda_x_axis),
                max=np.max(lambda_x_axis),
                step=0.001,
            ...
            right_edge=widgets.FloatSlider(
                min=np.min(lambda_x_axis),
                max=np.max(lambda_x_axis),
                step=0.001,
    ...
    def fitting(self):
        ...
        self.bhkl_layout = widgets.HBox(
            [
                widgets.Label("b\u2095\u2096\u2097"),
                widgets.IntText(default_bhkl, layout=widgets.Layout(width=text_width)),
            ]
        )
        low_layout = widgets.VBox([self.ahkl_layout, self.bhkl_layout])
        display(low_layout)
        ...
        self.fig4, self.ax4 = plt.subplots(figsize=(8, 8), nrows=1, ncols=1)
    ...
    def fit_peak(self, _):
        ...
        print(f"\u03bb\u2095\u2096\u2097: {lambdahkl:.3f}" + "\u212b")
        ax4.axvline(lambdahkl, color="b", linestyle="-.")
        _local_max_counts = np.max(-np.log(y_axis_to_fit))
        max_counts = max(max_counts, _local_max_counts)
        ax4.text(
            lambdahkl,
    ...
    def export_session(self, output_folder=None):
        ...
        for _key in rois_selected.keys():
            json_friendly_rois_selected[str(_key)] = {
                JSONKeys.x0: str(rois_selected[_key][JSONKeys.x0]),
        ...
        json_dict = {
            JSONKeys.infos: f"config file created on {current_time}",
        ...
        display(
            HTML(
                '<span>Created config file: </span><span style="font-size: 20px; color:blue">'
                + output_file_name
                + "</span>"
            )
        )
