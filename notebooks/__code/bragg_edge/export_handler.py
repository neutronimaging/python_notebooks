...
class ExportHandler:
    ...
    def makeup_name_of_profile_ascii_file(
        ...
        full_base_name = (
            "full_set_of_shrinkable_region_profiles_from_"
            + f"x{x0}_y{y0}_w{width}_h{height}_for_folder_{base_name}.txt"
        )
    ...
    def add_fitting_infos_to_metadata(self, metadata):
        ...
        for _key in self.parent.kropff_fitting_range.keys():
            metadata.append(
                f"#kropff {_key} selection range: [{self.parent.kropff_fitting_range[_key][0]}, {self.parent.kropff_fitting_range[_key][1]}]"
            )
        ...
        metadata.append(f"#march-dollase history init sigma: {sigma}")
        metadata.append(f"#march-dollase history init alpha: {alpha}")
    ...
    def make_metadata(
        ...
        for _row, _key in enumerate(dict_regions.keys()):
            ...
            metadata.append(
                f"#column {_key + 3} -> x0:{x0}, y0:{y0}, width:{width}, height:{height},"
                f" kropff: a0:{a0}, b0:{b0}, a0_error:{a0_error}, b0_error:{b0_error},"
                f" ahkl:{ahkl}, bhkl:{bhkl}, ahkl_error:{ahkl_error}, bhkl_error:{bhkl_error},"
                f" ldahkl:{ldahkl}, tau:{tau}, sigma:{sigma},"
                f" ldahkl_error:{ldahkl_error}, tau_error:{tau_error}, sigma_error:{sigma_error},"
                f" march_dollase: d_spacing:{d_spacing}, sigma:{sigma1}, alpha:{alpha},"
                f" a1:{a1}, a2:{a2}, a5:{a5}, a6:{a6},"
                f" d_spacing_error:{d_spacing_error}, sigma_error:{sigma1_error}, alpha_error:{alpha_error},"
                f" a1_error:{a1_error}, a2_error:{a2_error}, a5_error:{a5_error}, a6_error:{a6_error}"
            )
