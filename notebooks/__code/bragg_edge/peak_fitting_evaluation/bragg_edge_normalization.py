...
from __code.bragg_edge.peak_fitting_evaluation.bragg_edge import (
...
class BraggEdge(BraggEdgeParent):
    ...
    def load_data(self, folder_selected):
        ...
        if spectra_file:
            ...
            display(
                HTML(
                    '<span style="font-size: 15px; color:blue"> Spectra File automatically located: '
                    + self.spectra_file
                    + "</span>"
                )
            )
    ...
    def select_time_spectra_file(self):
        ...
        self.time_spectra_ui = ipywe.fileselector.FileSelectorPanel(
    ...
    def normalization_settings_widgets(self):
        ...
        vbox_with_ob = widgets.VBox(
            [
                self.select_ob_widget,
                spacer,
                hbox_1,
                spacer,
    ...
    def select_roi(self, nbr_data_to_use=2):
        self.o_interface = Interface(
            data=self.get_image_to_use_for_display(nbr_data_to_use=nbr_data_to_use),
    ...
    def select_ob_folder(self, status=None):
        select_data = ipywe.fileselector.FileSelectorPanel(
            instruction="Select OB Folder ...",
    ...
    def normalization_without_ob(self, list_rois):
        ...
        if list_rois is None:
            logging.info(
                "-> no ROIs found! At least one ROI must be provided. Normalization Aborted!"
            )
            display(
                HTML(
                    '<span style="font-size: 15px; color:red"> You need to provide a ROI!</span>'
                )
            )
            return
            ...
            for key in list_rois.keys():
                ...
                self.o_norm.normalization(
                    roi=list_o_roi, use_only_sample=True, notebook=True
                )
        display(
            HTML(
    ...
    def export_normalized_data_step2(self, output_folder):
        ...
        display(
            HTML(
                '<span style="font-size: 15px; color:green"> Created the normalized data in the folder '
    ...
    def plot(self):
        ...
        layout = go.Layout(
            ...
            yaxis=dict(title="Average Transmission"),
        )
    ...
    def export_data(self, output_folder):
        ...
        metadata = [f"# input folder: {input_folder}"]
        ...
        if len(list_roi) == 0:
            ...
            for index, key in enumerate(list_roi.keys()):
                ...
                _y1 = roi["y1"]
                metadata.append(
                    f"# ROI {index}: x0={_x0}, y0={_y0}, x1={_x1}, y1={_y1}"
                )
        metadata.append("#")
        ...
        if os.path.exists(output_file_name):
            display(
                HTML(
                    '<span style="font-size: 20px; color:blue">Ascii file '
                    + output_file_name
                    + " has been "
                    + "created  </span>"
                )
