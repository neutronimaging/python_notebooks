...
from __code.bragg_edge.fitting_functions import (
    march_dollase_advanced_fit,
    march_dollase_basic_fit,
)
...
class MarchDollaseFittingJobHandler:
    ...
    def initialize_fitting_input_dictionary(self):
        ...
        """
        logging.info("in initialize_fitting_input_dictionary")

        nbr_column = self.parent.ui.march_dollase_user_input_table.columnCount()
        list_name_of_parameters = []
        for _col in np.arange(nbr_column):
            _item = self.parent.ui.march_dollase_user_input_table.horizontalHeaderItem(
                _col
            ).text()
        ...
        logging.info(
            f"fitting_input_dictionary['rois'].keys(): {fitting_input_dictionary['rois'].keys()}"
        )
        ...
        for _row in fitting_input_dictionary["rois"].keys():
            ...
            if not d_spacing_flag:
                fitting_input_dictionary["rois"][_row]["fitting"]["march_dollase"][
                    "d_spacing"
                ] = d_spacing
    ...
    def isolate_left_center_right_axis(self, row=-1):
        ...
        [global_left_index, global_right_index] = [
            int(bragg_edge_range[0]),
        ...
        return {
            "left_part": {
                "lambda_x_axis": lambda_x_axis[0:left_index],
                "y_axis": y_axis[0:left_index],
            },
            "center_part": {
                "lambda_x_axis": lambda_x_axis[left_index:right_index],
                "y_axis": y_axis[left_index:right_index],
            },
            "right_part": {
                "lambda_x_axis": lambda_x_axis[right_index:],
    ...
    def get_a6(self):
        ...
        a6 = x_axis - (2.0 * y_axis) / (self.a5 - self.a2)
        self.a6 = a6
        return a6
    ...
    def prepare(self):
        ...
        def record_result_into_dict(
            entry_dict, result_object, name_of_parameter, parameter_flag
        ):
        ...
        for _roi_row in np.arange(nbr_roi_row):
            ...
            if _roi_row == 0:
                ...
                dict = {"xaxis": list(xaxis_float), "yaxis": list(yaxis_float)}
            ...
            for _history_row, _row_entry in enumerate(
                ...
                logging.info("---> Parameters pre-initialized:")
                if not d_spacing_flag:
                ...
                fitting_input_dictionary["rois"][_roi_row]["fitting"][
                    "march_dollase"
                ] = _entry
