...
class Kropff:
    def __init__(self, parent=None):
        ...
        self.table_ui = {
            "high_lda": self.parent.ui.high_lda_tableWidget,
    ...
    def export_bragg_peak_profile(self):
        ...
        _export_folder = QFileDialog.getExistingDirectory(
            self.parent, directory=working_dir, caption="Select Output Folder"
        )
        ...
        if _export_folder:
            ...
            for row_selected in list_row_selected:
                ...
                file_name = f"kropff_bragg_peak_profile_{name_of_row_formatted}.txt"
                ...
                metadata.append(f"# y0: {y0}")
                metadata.append(f"# width: {width}")
                metadata.append(f"# height: {height}")
                metadata.append(f"# a0: {a0}")
                metadata.append(f"# b0: {b0}")
                metadata.append(f"# ahkl: {ahkl}")
                metadata.append(f"# bhkl: {bhkl}")
                metadata.append("#")
    ...
    def update_fitting_plot(self):
        ...
        if list_row_selected is None:
            ...
            self.parent.fitting_peak_ui = pg.LinearRegionItem(
                values=peak_range, orientation="vertical", brush=None, movable=True, bounds=None
            )
            ...
            for row_selected in list_row_selected:
                ...
                self.parent.fitting_peak_ui = pg.LinearRegionItem(
                    values=peak_range, orientation="vertical", brush=None, movable=False, bounds=None
                )
                self.parent.fitting_peak_ui.sigRegionChanged.connect(self.parent.fitting_range_changed)
                self.parent.fitting_peak_ui.setZValue(-10)
                self.parent.ui.fitting.addItem(self.parent.fitting_peak_ui)
