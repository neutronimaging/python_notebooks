...
class NormalizationHandler:
    ...
    def export(self, rois={}):
        ...
        display(
            HTML(
                '<span style="font-size: 20px; color:blue">The normalized images are currently being '
                + 'created in </span><span style="font-size: 20px; color:green">'
                + output_folder
    ...
    def normalized(self, rois={}, output_folder=""):
        ...
        if not rois == {}:
            ...
            for _key in rois.keys():
                ...
                x1 = _roi["x1"]
                y1 = _roi["y1"]
                _list_roi.append(f"{x0},{y0},{x1},{y1}")
            str_list_roi = ":".join(_list_roi)
            command_line += f" -rois={str_list_roi}"

        # output
        output_folder = output_folder.replace(" ", "\\ ")
        command_line += f" --output={output_folder} &"
