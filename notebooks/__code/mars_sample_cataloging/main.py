...
from __code.mars_sample_cataloging.config import DEBUG, list_metadata_label_to_keep
...
class MarsSampleCataloging:
    ...
    def create_catalog(self, output_folder):
        ...
        self.extract_metadata_from_filenames(
            self.raw_dict_files["raw_radiographs"], "raw_radiographs"
        )
    ...
    def export_catalog_to_csv(self, output_folder):
        ...
        # raw radiographs
        raw_radiographs_df = pd.DataFrame.from_dict(
            self.metadata_dict["raw_radiographs"], orient="index"
        )
        output_file = os.path.join(
            output_folder, f"{self.ipts}_raw_radiographs_folder_sample_catalog.csv"
        )
        raw_radiographs_df.to_csv(output_file, index=False)
        logging.info(f"Raw radiographs catalog exported to: {output_file}")
        display(
            HTML(
                f"""<h3>Raw Radiographs catalog exported to: <b>{output_file}</b></h3>"""
            )
        )

        # ct_scans
        ct_scans_df = pd.DataFrame.from_dict(
            self.metadata_dict["ct_scans"], orient="index"
        )
        output_file = os.path.join(
            output_folder, f"{self.ipts}_ct_scans_folder_sample_catalog.csv"
        )
        ct_scans_df.to_csv(output_file, index=False)
        logging.info(f"CT scans catalog exported to: {output_file}")
        display(
            HTML(f"""<h3>CT Scans catalog exported to: <b>{output_file}</b></h3>""")
        )
    ...
    def calculate_and_plot_statistics_for_category(self, category):
        data_frame = pd.DataFrame.from_dict(
            self.metadata_dict[category], orient="index"
        )
        ...
        if category == "raw_radiographs":
            ...
            data_frame["Acquisition Time (s)"] = pd.to_numeric(
                data_frame["Acquisition Time (s)"], errors="coerce"
            )
            fig = px.histogram(
                data_frame,
        ...
        if category == "ct_scans":
            ...
            data_frame["Number of Files in Folder"] = pd.to_numeric(
                data_frame["Number of Files in Folder"], errors="coerce"
            )
            fig = px.histogram(
                data_frame,
            ...
            data_frame["Total Acquisition Time (s)"] = pd.to_numeric(
                data_frame["Total Acquisition Time (s)"], errors="coerce"
            )
            fig = px.histogram(
                data_frame,
        ...
        display(
            HTML(
                f"""<p><b>Number of unique samples</b> in <b>{category}</b>: {num_unique_samples}</p>"""
            )
        )
            ...
            with tifffile.TiffFile(tiff_file) as tif:
                ...
                for tag in page.tags:
                    ...
                    if _metadata_label in list_metadata_label_to_keep:
                        if (
                            _metadata_label == "65000"
                        ):  # this is the acquisition timestamp in seconds
                            _metadata_value_formated = MetadataHandler.convert_to_human_readable_format(
                            ...
                            _metadata_label == "JEOL_Header"
                        ):  # this is a long string, so we only log the first 50 characters
    ...
    def extract_metadata_from_filenames(self, file_list, category):
        ...
        with self.output:
            display(
                HTML(
                    f"""Extracting metadata from TIFF for category: <b>{category}</b> ... </h2>"""
                )
            )
        ...
        with self.output:
            ...
            display(
                HTML(
                    f"""Metadata extraction for category: <b>{category}</b> completed."""
                )
            )
    ...
    def extract_metadata_from_folders(self, folder_list, category):
        ...
        with self.output:
            display(
                HTML(
                    f"""Extracting metadata from folders for category: <b>{category}</b> ... </h2>"""
                )
            )
        ...
        with self.output:
            for _folder in tqdm(folder_list):
                ...
                _metadata_of_that_folder = self._retrive_metadata_from_tiff_file(
                    first_file
                )
                local_metadata_dict[_folder].update(_metadata_of_that_folder)
                local_metadata_dict[_folder][TagNames["number of files in folder"]] = (
                    nbr_files
                )
                local_metadata_dict[_folder][TagNames["total acquisition time (s)"]] = (
                    nbr_files * float(_metadata_of_that_folder[TagNames["JEOL_Header"]])
                )
                local_metadata_dict[_folder][TagNames["first file in folder"]] = (
                    first_file
                )
            ...
            display(
                HTML(
                    f"""Metadata extraction from folders for category: <b>{category}</b> completed."""
                )
            )
    ...
    def retrieve_all_tiff_files_from_ipts_raw_radiographs(self) -> dict:
        ...
        if DEBUG:
            ...
            logging.info(
                f"\tDEBUG mode is ON - Limiting number of files to: {len(raw_tiff_files)}"
            )

        return {"raw_radiographs": raw_tiff_files}

    def retrieve_all_folders_from_ipts_ct_scans(self) -> dict:
        ct_search_path = os.path.join(self.working_dir, "raw", "ct_scans")
        logging.info(
            f"Searching for all the folders in ct_scans subfolder: {ct_search_path}"
        )
        ...
        ct_scans_folders = [
            folder for folder in ct_scan_folders if os.path.isdir(folder)
        ]
