...
class DehydrationHydrationCorrection:
    ...
    def load_images_from_folder(self, folder_name):
        ...
        display(
            HTML(
                "<span>Number of TIFF images found in the folder: "
                + str(len(list_of_images))
                + "</span>"
            )
        )
        ...
        if len(list_of_images) == 0:
            logging.warning(
                "No TIFF images found in the selected folder. Let's look one level deeper in the folder structure."
            )
            display(
                HTML(
                    "<span style='color: red;'>No TIFF images found in the selected folder. Let's look one level deeper in the folder structure.</span>"
                )
            )
            ...
            list_of_images.sort()
            logging.info(
                f"After searching subfolders, number of TIFF images found: {len(list_of_images)}"
            )
