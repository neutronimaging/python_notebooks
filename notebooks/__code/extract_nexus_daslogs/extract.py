...
from __code.nexus_handler import get_entry_value, get_list_entries
...
class Extract(FileFolderBrowser):
    ...
    def select_nexus(self):
        o_file = FileFolderBrowser(
            working_dir=self.working_dir, next_function=self.select_metadata
        )
        o_file.select_images(
            instruction="Select the NeXus files to extract infos from!",
            multiple_flag=True,
    ...
    def select_metadata(self, list_nexus=None):
        ...
        self.dict_daslogs_keys = get_list_entries(
            nexus_file_name=self.first_nexus_selected, starting_entries=STARTING_ENTRIES
        )
    ...
    def display_widgets(self):
        ...
        with h5py.File(nexus_path, "r") as nxs:
            value_of_first_selected_element = list(
                nxs["entry"]["DASlogs"][first_daslogs_key][first_key]
            )
        ...
        hori_box = widgets.HBox(
            [
                widgets.Select(
                    options=self.list_daslogs_keys,
        ...
        interpolate_hbox = widgets.HBox(
            [
                widgets.Checkbox(value=False, description="Interpolate y-axis"),
                widgets.Label(value="using x-axis increment values of"),
                widgets.FloatText(value=10, layout=widgets.Layout(width="20%")),
            ]
        )
        ...
        self.x_axis_intermediate_key_widget.observe(
            self.x_axis_intermediate_value_changed, names="value"
        )
        self.y_axis_intermediate_key_widget.observe(
            self.y_axis_intermediate_value_changed, names="value"
        )
        ...
        self.interpolate_checkbox.observe(
            self.interpolate_checkbox_changed, names="value"
        )
    ...
    def x_axis_intermediate_value_changed(self, value=None):
        ...
        with h5py.File(self.first_nexus_selected, "r") as nxs:
            value_of_first_selected_element = list(
                nxs["entry"]["DASlogs"][first_daslogs_key][new_intermediate_key]
            )
    ...
    def extract_all(self, output_folder):
        ...
        Extract.create_output_file(file_name=output_file_name, dictionary=final_dict)
    ...
    def makeup_output_file_name(self):
        ...
        return str(
            Path(output_folder)
            / (
                f"{nbr_nexus}_nexus_metadata_"
                + f"{top_key}_{x_axis_key}_{y_axis_key}"
                + ".txt"
            )
        )
    ...
    def extract(
        self,
        nexus_file_name="",
        top_key_widget_value=None,
        x_axis_key=None,
        y_axis_key=None,
    ):
        top_key_widget_value = (
            top_key_widget_value if top_key_widget_value else self.top_keys_widget_value
        )
        x_axis_key = (
            x_axis_key if x_axis_key else self.x_axis_intermediate_key_widget.value
        )
        y_axis_key = (
            y_axis_key if y_axis_key else self.y_axis_intermediate_key_widget.value
        )
        interpolate_flag = self.interpolate_checkbox.value
        interpolate_increment_value = (
            self.interpolate_value.value if interpolate_flag else None
        )
        ...
        metadata.append(f"# interpolated y_axis: {interpolate_flag}")
        if interpolate_flag:
            metadata.append(f"# x-axis increment value: {interpolate_increment_value}")
        ...
        x_axis_array = get_entry_value(
            nexus_file_name=nexus_file_name, entry_path=x_axis_entry_path
        )
        y_axis_entry_path = copy.deepcopy(top_entry_path)
        y_axis_entry_path.append(y_axis_key)
        y_axis_array = get_entry_value(
            nexus_file_name=nexus_file_name, entry_path=y_axis_entry_path
        )
        ...
        if interpolate_flag:
            ...
            new_x_axis_array = np.arange(
                x_min, x_axis_array[-1], interpolate_increment_value
            )
            o_interpolation = Interpolation(x_axis=x_axis_array, y_axis=y_axis_array)

            try:
                y_axis_array = o_interpolation.get_new_y_array(
                    new_x_axis=new_x_axis_array
                )
            except TypeError:
                return None
        ...
        col3 = {"data": None, "name": "absolute time"}
        col4 = {"data": None, "name": "master relative time (s)"}
        col_legend = f"{x_axis_key}, {y_axis_key}"
        if use_absolute_time_offset:
            starting_time = get_entry_value(
                nexus_file_name=nexus_file_name, entry_path=["entry", "start_time"]
            )
            starting_time = starting_time[0].decode("UTF-8")
            metadata.append(f"# starting time of this file: {starting_time}")

            master_starting_time = get_entry_value(
                nexus_file_name=self.first_nexus_selected,
                entry_path=["entry", "start_time"],
            )
            master_starting_time = master_starting_time[0].decode("UTF-8")
            metadata.append(f"# starting time of first file: {master_starting_time}")
            ...
            absolute_time_axis = o_absolute.get_absolute_time_for_this_delta_time_array(
                delta_time_array=time_axis
            )
            col3["data"] = absolute_time_axis
            col_legend += ", {}".format("absolute time")

            o_relative = RelativeTimeHandler(
                master_initial_time=master_starting_time,
                local_initial_time=starting_time,
            )
            master_relative_time_axis = (
                o_relative.get_relative_time_for_this_time_array(time_array=time_axis)
            )
            col4["data"] = master_relative_time_axis
            col_legend += ", {}".format("master relative time (s)")

        metadata.append("#")
        metadata.append(f"# {col_legend}")

        return {
            "col1": {"data": x_axis_array, "name": x_axis_key},
            "col2": {"data": y_axis_array, "name": y_axis_key},
    ...
    def create_output_file(file_name=None, dictionary=None):
        if len(dictionary) == 0:
            display(
                HTML(
                    '<span style="font-size: 20px; color:red">No ASCII file has been created!</span>'
                )
            )
            return
        ...
        with open(file_name, "w") as f:
            for _key in dictionary.keys():
                ...
                for _index in np.arange(len(item["col1"]["data"])):
                    _line = f"{item_col1[_index]}, {item_col2[_index]}"
                    if item_col3:
                        _line += f", {item_col3[_index]}, {item_col4[_index]}"
                    _line += "\n"
                    f.write(_line)
