...
class Extract(FileFolderBrowser):
    ...
    def makeup_output_file_name(self):
        ...
        return str(
            Path(output_folder) / (f"{nbr_nexus}_nexus_metadata_" + f"{top_key}_{x_axis_key}_{y_axis_key}" + ".txt")
        )
    ...
    def extract(self, nexus_file_name="", top_key_widget_value=None, x_axis_key=None, y_axis_key=None):
        ...
        x_axis_array = get_entry_value(nexus_file_name=nexus_file_name, entry_path=x_axis_entry_path)
        y_axis_entry_path = copy.deepcopy(top_entry_path)
        y_axis_entry_path.append(y_axis_key)
        y_axis_array = get_entry_value(nexus_file_name=nexus_file_name, entry_path=y_axis_entry_path)
        ...
        if use_absolute_time_offset:
            starting_time = get_entry_value(nexus_file_name=nexus_file_name, entry_path=["entry", "start_time"])
            ...
            master_starting_time = get_entry_value(
                nexus_file_name=self.first_nexus_selected, entry_path=["entry", "start_time"]
            )
            ...
            o_relative = RelativeTimeHandler(master_initial_time=master_starting_time, local_initial_time=starting_time)
