...
from __code.ipywe.myfileselector import (
    FileSelectorPanelWithJumpFolders as MyFileSelectorPanelWithJumpFolders,
)
...
class NormalizationTof:
    ...
    def display_infos(
        ...
        spectra_file_found=True,
        correct_chips_alignment_flag=None,
    ):
                ...
                if os.path.exists(_file_full_path):
                    ...
                    if is_valid_run:
                        ...
                        _is_spectra_file_found, spectra_file_name = (
                            NormalizationTof._is_spectra_file_found_and_list(
                                _file_full_path
                            )
                        )
                        ...
                        _is_summary_json_file_found, _summary_dict = (
                            NormalizationTof._is_summary_json_file_found(
                                _file_full_path
                            )
                        )
                        notebook_logging.info(
                            f"\tSummary JSON file found: {_is_summary_json_file_found}"
                        )
                        notebook_logging.info(
                            f"\tSummary JSON content: {_summary_dict}"
                        )
                        correct_chips_alignment_flag = (
                            _summary_dict.get("chips_alignment_correction", None)
                            if _is_summary_json_file_found
                            else None
                        )
            ...
            for _run in self.sample_run_numbers_selected:
                ...
                if os.path.exists(_run):
                    ...
                    if is_valid_run:
                        ...
                        _is_spectra_file_found, spectra_file_name = (
                            NormalizationTof._is_spectra_file_found_and_list(_run)
                        )
                        if not _is_spectra_file_found:
                            self.spectra_file_found = False
                        else:
                        ...
                        _is_summary_json_file_found, _summary_dict = (
                            NormalizationTof._is_summary_json_file_found(_run)
                        )
                        notebook_logging.info(
                            f"\tSummary JSON file found: {_is_summary_json_file_found}"
                        )
    ...
    def check_ob(self):
            ...
            for _file_full_path in list_of_ob_full_path:
                if os.path.exists(_file_full_path):
                    ...
                    if is_valid_run:
                        ...
                        notebook_logging.info(
                            f"\tOB full path {_file_full_path} - FOUND with {nbr_tiff} tif* files"
                        )
                        display(
                            HTML(
                                f"<span style='color:green'>{_file_full_path}</span> - OK"
                    ...
                    notebook_logging.info(
                        f"\tOB full path {_file_full_path} - NOT FOUND"
                    )
                    display(
                        HTML(
                            f"<span style='color:red'>{_file_full_path} - NOT FOUND!</span>"
        ...
        if len(set(self.check_nbr_tiff[DataType.ob])) > 1:
            ...
            len(self.check_nbr_tiff[DataType.ob]) == 0
        ):  # check_nbr_tiff[DataType.ob] is not empty
            display(HTML("<span style='color:red'>Empty OB run selected!</span>"))
            notebook_logging.info("WARNING: Not valid OB runs found!")

        else:
            if (
                len(self.check_nbr_tiff[DataType.sample]) > 0
                and len(self.check_nbr_tiff[DataType.ob]) > 0
            ):
                if (
    ...
    def _load_and_get_integrated_ob(self, full_path):
        ...
        display(
            HTML(
                f"<span style='color:blue'>Loading integrated OB data for {os.path.basename(full_path)} ... </span>"
            )
        )
        ...
        display(
            HTML(
                f"<span style='color:green'>Integrated OB data loaded for {os.path.basename(full_path)}!</span>"
            )
        )
    ...
    def output_folder_selected(self, folder_selected):
        ...
        with self.list_input_folders_ui.out:
            ...
            if os.path.exists(folder_selected):
                ...
                notebook_logging.info(
                    f"Output folder selected: {folder_selected} - FOUND"
                )
            else:
                display(
                    HTML(
    ...
    def profile_of_roi(self):
        ...
        def plot_normalized_profile_of_roi(index, left=0, top=0, width=50, height=50):
            ...
            fig = make_subplots(
                ...
                cols=2,
                subplot_titles=[
                    f"Integrated normalized data - {index}",
                    "",
                    f"Profile of ROI - {index}",
                    "",
                ],
                specs=[
            ...
            fig.add_trace(
                go.Scatter(x=lambda_array, y=_profile, mode="lines", name="Profile"),
                row=2,
                col=1,
            )
