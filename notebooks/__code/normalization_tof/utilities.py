...
def preview_normalized_data(
    ...
    fig2.add_trace(
        go.Scatter(
            y=ob_integrated, mode="markers", marker=dict(size=MARKERSIZE), name="OB"
        ),
        row=1,
        col=2,
    )
    ...
    if dc_data_combined is not None:
        ...
        fig3.add_trace(
            go.Scatter(
                y=dc_integrated, mode="markers", marker=dict(size=MARKERSIZE), name="DC"
            ),
            row=1,
            col=2,
        )
    ...
    vmin, vmax = np.percentile(
        normalized_data_integrated[~np.isnan(normalized_data_integrated)], [2, 98]
    )
    ...
    fig4.add_trace(
        go.Scatter(
            y=profile, mode="markers", marker=dict(size=MARKERSIZE), showlegend=False
        ),
        row=1,
        col=2,
    )
    ...
    if lambda_array is not None:
        fig5 = make_subplots(
            ...
            cols=2,
            subplot_titles=[
                f"{_label.split('profile of ')[-1]} counts vs Lambda",
        ...
        if _spectrum_normalized_data is not None:
            ...
            fig6.add_trace(
                go.Scatter(
                    x=lambda_array,
                    y=_spectrum_normalized_data,
                    mode="markers",
                    marker=dict(symbol="star", size=MARKERSIZE, color="red"),
                    name="spectrum normalization of ROI",
                ),
                row=1,
                col=1,
            )
            ...
            fig6.update_layout(
                height=600, width=1200, margin=dict(l=50, r=50, t=80, b=50)
            )
            fig6.show()
...
def init_master_dict(data_dictionary: dict) -> dict:
    ...
    for _base_name in data_dictionary:
...
def update_dict_with_spectra_files(
    ...
    for _run_number in master_dict:
        if spectra_array is not None:
...
def combine_dc_images(dc_master_dict: dict) -> np.ndarray:
    ...
    for _dc_run_number in dc_master_dict:
...
def combine_ob_images(
    ...
    if use_proton_charge:
        ...
        list_proton_charges = []
        for _ob_run_number in ob_master_dict:
...
def export_normalized_data(
    ...
    list_ob_folder_name = list(
        ob_master_dict.keys()
    )  # those are actually full path, not run number
    list_ob_runs = [
        ob_master_dict[_ob_folder_name][MasterDictKeys.run_number]
        for _ob_folder_name in list_ob_folder_name
    ]
