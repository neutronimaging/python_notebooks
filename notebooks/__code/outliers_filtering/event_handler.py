...
class EventHandler:
    ...
    def reset_table_infos(self):
        ...
        for _row in np.arange(nbr_row):
            for _col in np.arange(1, 5):
                o_table.insert_item(row=_row, column=_col, editable=False, value="")
    ...
    def load_raw_data(self, row=0):
        ...
        o_norm.load(file=file, auto_gamma_filter=False, manual_gamma_filter=False)
        _raw_data = np.squeeze(o_norm.data["sample"]["data"][0])
        return _raw_data

    def load_data_object(self, file_name=None):
        o_norm = Normalization()
        o_norm.load(file=file_name, auto_gamma_filter=False, manual_gamma_filter=False)
        return o_norm

    def calculate_filtered_data(self, raw_data=None):
        o_algo = Algorithm(parent=self.parent, data=raw_data)
        o_algo.run()
    ...
    def mouse_moved_in_any_image(self, position=None, image=None):
        ...
        if image_view.view.sceneBoundingRect().contains(pos):
            ...
            if (0 <= mouse_x < width) and (0 <= mouse_y < height):
                ...
                self.parent.raw_value.setText(f"{_raw_value:.02f}")
                self.parent.filtered_value.setText(f"{_filtered_value:.02f}")
