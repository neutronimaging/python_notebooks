    ...
    with open(file_name) as f:
        for line in f.readlines():
            ...
            if line.startswith("#List of files"):
                matching_pattern = r"\#\w*\s{1}\w*\s{1}\w*\s{1}\((\d*)"
...
def loading_radial_profile_file(list_of_ascii_files):
    ...
    display(
        HTML(
            '<span style="font-size: 20px; color:blue">Notebooks successfully loaded the '
            + nbr_files
    ...
    return {
        "list_of_ascii_files": list_of_ascii_files,
