...
from __code.wave_front_dynamics.loader import (
    loading_linear_profile_file,
    loading_radial_profile_file,
)
...
class WaveFrontDynamics:
    ...
    list_of_data = None
    input_ascii_file_type = InputASCIIType.radial_profile
    ...
    def load_data(self):
        ...
        self.input_ascii_file_type = self.determine_input_ascii_file_type(
            list_of_ascii_files[0]
        )
        if self.input_ascii_file_type == InputASCIIType.linear_profile:
            if len(list_of_ascii_files) > 1:
                display(
                    HTML(
                        "<font color='red'>Select only 1 ASCII file generated by the linear profile notebook!"
                    )
                )
                return
            result_dict = loading_linear_profile_file(list_of_ascii_files[0])
            display(HTML("Loaded using linear profile method .... Done!"))
    ...
    def select_data(self):
        instructions = (
            "Select linear profile ASCII or Radial Profile list of ascii files ..."
        )
...
class WaveFrontDynamicsUI(QMainWindow):
    ...
    def __init__(self, parent=None, working_dir="./", wave_front_dynamics=None):
        display(
            HTML(
                '<span style="font-size: 20px; color:blue">Launched UI! '
        ...
        self.input_ascii_file_type = (
            wave_front_dynamics.input_ascii_file_type
        )  # linear or radial profile
        self.list_of_data = wave_front_dynamics.list_of_data
        self.list_timestamp = wave_front_dynamics.list_timestamp
        self.list_of_original_image_files = (
            wave_front_dynamics.list_of_original_image_files
        )
        self.list_of_original_image_files_to_use = copy.deepcopy(
            wave_front_dynamics.list_of_original_image_files
        )
        ...
        if self.list_of_data is None:
            display(
                HTML(
                    '<span style="font-size: 20px; color:red">No ASCII file loaded!</span>'
                )
            )
            return
        ...
        ui_full_path = os.path.join(
            ...
            os.path.join("ui", "ui_wave_front_dynamics.ui"),
        )
        ...
        self.list_relative_timestamp = [
            float(_time) - t0 for _time in self.list_timestamp
        ]
