...
class DisplayFileNamesVsTimeStamp:
    ...
    def print_result(self):
        ...
        for _index, _file in enumerate(self.list_files):
            ...
            _text = f"{_short_file} -> {_relative_time}"
        ...
        for _index, _file in enumerate(self.list_files):
            ...
            _text = f"{_short_file} -> {_absolute_time}"
            absolute_text_area.append(_text)
        absolute_text_area = "\n".join(absolute_text_area)

        children = [
            widgets.Textarea("", layout=widgets.Layout(width="100%", height="300px")),
            widgets.Textarea("", layout=widgets.Layout(width="100%", height="300px")),
            widgets.Textarea("", layout=widgets.Layout(width="100%", height="300px")),
        ]
        tab = widgets.Tab()
    ...
    def export(self):
        ...
        for _index, _file in enumerate(file_list):
            text += f"{os.path.abspath(_file)}, {time_stamp[_index]}, {time_stamp_user_format[_index]}, {time_offset[_index]}\n"
