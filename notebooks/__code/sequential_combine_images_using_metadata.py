...
class SequentialCombineImagesUsingMetadata:
    working_dir = ""
    ...
    def are_metadata_within_error_range(self, metadata1, metadata2):
        ...
        for _key in metadata1.keys():
            if (
                np.abs(
                    self.isolate_value_from_metadata(metadata1[_key])
                    - self.isolate_value_from_metadata(metadata2[_key])
                )
                > METADATA_ERROR
            ):
                return False
    ...
    def recap_merging_list(self):
        box0 = widgets.VBox(
            [
                widgets.Label("List of Runs", layout=widgets.Layout(width="100%")),
                widgets.Select(
                    options=self.master_list_images_to_combine.keys(),
        ...
        box1 = widgets.VBox(
            [
                widgets.Label("List of positions", layout=widgets.Layout(width="100%")),
                widgets.Select(
                    options=self.get_list_of_position_for_select_run(),
                    layout=widgets.Layout(width="150px", height="300px"),
                ),
            ],
            layout=widgets.Layout(width="160px"),
        ...
        box3 = widgets.VBox(
            [
                ...
                widgets.Label(f"Error allowed: {METADATA_ERROR}"),
            ],
    ...
    def _define_merged_file_name(
        ...
        """
        return os.path.join(output_folder, f"{run_label}_{position_label}.tiff")
