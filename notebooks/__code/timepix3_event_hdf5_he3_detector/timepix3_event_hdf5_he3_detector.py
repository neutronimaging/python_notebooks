...
class Timepix3EventHdf5:
    ...
    def rebin_and_display_h3_data(self):
        hbox = widgets.HBox(
            [
                widgets.Label("Bin size:", layout=widgets.Layout(width="100px")),
                widgets.Label("NaN", layout=widgets.Layout(width="200px")),
            ]
        )
        display(hbox)
        ...
        def plot_rebinned_data(
            ...
            if x_axis == "TOF":
                ...
                _exp = Experiment(
                    tof=bins_array[:-1] * 1e-6,  # to convert to seconds
