...
import numpy as np
import pandas as pd
from __code._utilities.is_it import is_it_a_folder
...
from __code.normalization_tof import DataType, Roi
from __code.normalization_white_beam_at_venus import (
    DataDict,
    DataType,
    DetectorType,
    FolderPath,
...
from __code.normalization_white_beam_at_venus.utilities import (
    extract_metadata,
    extract_run_number_from_file_name,
)
...
class NormalizationWhiteBeamAtVenus:
    ...
    def initialize(self, working_dir=None, debug=False):
        ...
        file_name = "normalization_white_beam_at_venus"
        ...
        display(
            HTML(f"<span style='color:green'>log book created: {log_file_name}</span>")
        )
    ...
    def setup_paths(self):
        ...
        if debug:
            self.folder_path.sample = (
                Path(DEBUG_DATA.working_dir)
                / Path("images")
                / Path(self.detector_type.value)
                / Path("raw/radiography")
            )
            self.folder_path.ob = (
                Path(DEBUG_DATA.working_dir)
                / Path("images")
    ...
    def check_data(
        ...
        if run_numbers_widget_value.strip() == "":
            ...
            notebook_logging.info(
                f"{data_type.value} selection: {run_numbers_widget_value}"
            )

            if files_or_folders_selected is None:
                display(
                    HTML(
                        f"<span style='color:red'>No {data_type.value} runs/folders selected!</span>"
                    )
                )
                return
            ...
            for _selection in files_or_folders_selected:
                ...
                if is_it_a_folder(_selection):
                    notebook_logging.info(
                        f"\t{_selection} is a folder, retrieving tiff files in that folder..."
                    )
                    # concatenate list_of_full_path_images with the existing list_of_full_path_images
                    list_of_full_path_images.extend(
                        glob.glob(os.path.join(_selection, "*.tif*"))
                    )

                else:
                    notebook_logging.info(
                        f"\t{_selection} is a file, checking if it's a tiff file..."
                    )
                    if _selection.endswith(".tif") or _selection.endswith(".tiff"):
                        notebook_logging.info(
                            f"\t{_selection} is a tiff file, adding to the list of images..."
                        )
                        list_of_full_path_images.append(_selection)

            logging.info(
                f"\tTotal number of tiff files found: {len(list_of_full_path_images)}"
            )
            logging.info("\tExtracting run numbers from the file names...")
            ...
            if len(list_of_images_rejected) > 0:
                display(
                    HTML(
                        f"<span style='color:orange'>List of images rejected (no run number found in the file name): {list_of_images_rejected}</span>"
                    )
                )

            logging.info(
                f"List of images rejected (no run number found in the file name): {list_of_images_rejected}"
            )
            logging.info(
                f"{data_type} run numbers extracted from the file names: {list_of_runs}"
            )
        ...
        display(
            HTML(
                f"<span style='color:green'>Sample run numbers selected: {list_of_runs}</span>"
            )
        )
    ...
    def settings(self):
        ...
        label = widgets.Label(
            value="Distance source detector (m)", layout=widgets.Layout(width="150px")
        )
        ...
        if self.instrument == "SNAP":
            label = widgets.Label(
                value="Detector offset (us)", layout=widgets.Layout(width="150px")
            )
            self.detector_offset_us = widgets.FloatText(
                value=0.0, disabled=False, layout=widgets.Layout(width="150px")
            )
    ...
    def what_to_export(self):
        ...
        if not (self.spectra_array is None) or (
