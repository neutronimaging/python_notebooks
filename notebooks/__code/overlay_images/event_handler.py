...
class EventHandler:
    ...
    def update_profile_markers_and_target(self, with_profile=False):
        ...
        if with_profile:
            if self.parent.markers["overlay"]["1"]["target_ui"] is not None:
            ...
            lines = np.array(
                [line_color for _ in np.arange(len(adj))],
                dtype=[
                    ("red", np.ubyte),
                    ("green", np.ubyte),
                    ("blue", np.ubyte),
    ...
    def get_marker_index_parameters(self, region_index="1"):
        region = {
            "high_res": {
                "x": self.parent.markers["high_res"][region_index]["x"],
                "y": self.parent.markers["high_res"][region_index]["y"],
            },
            "low_res": {
                "x": self.parent.markers["low_res"][region_index]["x"],
    ...
    def overlay_stack_of_images_clicked(self):
        ...
        message = f"Overlay created using a scaling factor of {scaling_factor:.2f}!"
    ...
    def manual_overlay_stack_of_images_clicked(self):
        ...
        message = "Overlay created using manual settings!".format()
        self.parent.ui.statusbar.showMessage(message, 10000)  # 10s
        self.parent.ui.statusbar.setStyleSheet("color: green")
    ...
    def transparency_widgets_status(self):
        ...
        list_ui = [
            self.parent.ui.low_resolution_label,
