...
class EventHandler:
    ...
    def list_folder_combobox_value_changed(self, new_folder_selected=None):
        ...
        for _row_index, _file in enumerate(list_files):
            ...
            o_table.insert_widget(row=_row_index, column=3, widget=cell_widget)
    ...
    def load_table(self):
        table_file_name = QFileDialog.getOpenFileName(
            self.parent,
        ...
        if table_file_name:
            with open(table_file_name) as read_file:
            ...
            if table_ui_nbr_row != input_table_nbr_row:
                show_status_message(
                    parent=self.parent,
                    message="Size of table and loaded file DO NOT MATCH!",
                    status=StatusMessageStatus.error,
                    duration_s=10,
                )
                return

            WidgetsHandler.block_signals(ui=o_table.table_ui, status=True)

            for _row in np.arange(table_ui_nbr_row):
                xoffset, yoffset = table_data[str(_row)]
                o_table.set_item_with_str(row=_row, column=1, cell_str=xoffset)
                self.save_table_offset_of_this_cell(_row, 1)
                o_table.set_item_with_str(row=_row, column=2, cell_str=yoffset)
                self.save_table_offset_of_this_cell(_row, 2)
                QApplication.processEvents()

            WidgetsHandler.block_signals(ui=o_table.table_ui, status=False)
    ...
    def export_table(self, table_file_name=""):
        if table_file_name == "":
            table_file_name = QFileDialog.getSaveFileName(
                self.parent,
