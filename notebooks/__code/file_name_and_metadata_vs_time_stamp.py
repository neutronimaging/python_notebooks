...
class FileNameMetadataTimeStamp:
    ...
    def select_metadata_file(self):
        self.sample_environment_file_ui = ipywe.fileselector.FileSelectorPanel(
            instruction="Select Sample Environment File ...", start_dir=self.working_dir
        )
    ...
    def __extract_metadata(self, index=-1, metadata_array=[], time_stamp_array=[]):
        [left_meta, left_index] = self.__get_first_metadata_and_index_value(
            index=index, data_array=metadata_array, direction="left"
        )
        [right_meta, right_index] = self.__get_first_metadata_and_index_value(
            index=index, data_array=metadata_array, direction="right"
        )
    ...
    def export(self):
        ...
        pprint(f"Created File: {output_file_name}")
