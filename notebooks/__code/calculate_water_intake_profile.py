...
class CalculateWaterIntakeProfile:
    ...
    def select_counts_vs_time_files(self):
        self.list_files_ui = ipywe.fileselector.FileSelectorPanel(
            instruction="Select Counts vs Time Files ...", start_dir=self.working_dir, multiple=True
        )
    ...
    def retrieve_data(self, file_name, nbr_comment_line=0, col_names=None):
        file_0 = pd.read_csv(file_name, skiprows=nbr_comment_line, header=None, names=col_names)
        return file_0
    ...
    def retrieve_data_and_metadata(self):
        ...
        for index, file in enumerate(self.list_files):
            ...
            _data = self.retrieve_data(file, nbr_comment_line=_metadata["nbr_comment_line"], col_names=_new_col_names)
    ...
    def display_water_intake_vs_profile(self):
        ...
        def display_intake_vs_profile(file_index):
            ...
            m = re.match(r"\((?P<y0>\d*), (?P<x0>\d*), (?P<height>\d*), (?P<width>\d*)\)", _metadata_entry)
            y0 = float(m.group("y0"))
        ...
        _ = interact(
            ...
            file_index=widgets.IntSlider(min=0, max=len(list_files) - 1, value=0, description="File Index"),
        )
    ...
    def define_pixel_size(self):
        box = widgets.HBox(
            [
                ...
                widgets.FloatText(value=1, step=0.1, layout=widgets.Layout(width="10%")),
                widgets.Label("mm/px"),
            ]
        )
