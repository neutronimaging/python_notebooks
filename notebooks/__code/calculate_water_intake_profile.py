...
class CalculateWaterIntakeProfile:
    files_data = []
    files_metadata = []

    documentation_path = (
        "__docs/calculate_water_intake_profile/water_intake_calculation.pdf"
    )
    ...
    def select_counts_vs_time_files(self):
        self.list_files_ui = ipywe.fileselector.FileSelectorPanel(
            instruction="Select Counts vs Time Files ...",
    ...
    def retrieve_data(self, file_name, nbr_comment_line=0, col_names=None):
        file_0 = pd.read_csv(
            file_name, skiprows=nbr_comment_line, header=None, names=col_names
        )
        return file_0
    ...
    def calculate_water_intake_peak(self):
        ...
        for _index, _file in enumerate(self.list_files):
            ...
            delta_array = []
            for _pixel in range(nbr_pixel - 1):
    ...
    def display_water_intake_vs_profile(self):
        ...
        def display_intake_vs_profile(file_index):
            ...
            m = re.match(
                r"\((?P<y0>\d*), (?P<x0>\d*), (?P<height>\d*), (?P<width>\d*)\)",
                _metadata_entry,
            )
            y0 = float(m.group("y0"))
        ...
        _ = interact(
            display_intake_vs_profile,
            file_index=widgets.IntSlider(
                min=0, max=len(list_files) - 1, value=0, description="File Index"
            ),
        )
    ...
    def define_pixel_size(self):
        box = widgets.HBox(
            [
                widgets.Label("Size/Pixel Ratio"),
                widgets.FloatText(
                    value=1, step=0.1, layout=widgets.Layout(width="10%")
                ),
                widgets.Label("mm/px"),
            ]
        )
    ...
    def export(self):
        ...
        short_file_name = (
            os.path.basename(os.path.abspath(os.path.dirname(self.list_files[0])))
            + ".txt"
        )
        output_file_name = os.path.join(output_folder, short_file_name)

        ascii_text = (
            f"# Source data: {os.path.basename(os.path.dirname(self.list_files[0]))}\n"
        )
        ...
        for _index in range(len(self.time_stamps)):
            ...
            ascii_text += f"{_x_value}, {_y_value}\n"
