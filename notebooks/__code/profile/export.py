...
class ExportProfiles:
    ...
    def _create_output_file_name(self, profile_index=0):
        ...
        output_file_name = os.path.join(
            self.export_folder,
            f"{base_name}_profile_{profile_index + 1}_{get_current_time_in_special_file_name_format()}.txt",
        )
...
class ExportAverageROI:
    ...
    def _create_output_file_name(self, roi_index=0):
        ...
        output_file_name = os.path.join(
            self.export_folder,
            f"{base_name}_mean_counts_{roi_index + 1}_{get_current_time_in_special_file_name_format()}.txt",
        )
        return output_file_name
    ...
    def run(self):
        ...
        for _roi_index in np.arange(_nbr_profiles):
            ...
            for _index, _data in enumerate(self.parent.data_dict["data"]):
                ...
                _data_output.append(f"{_basename}, {_mean}")

            make_ascii_file(
                metadata=[
                    "# Mean of ROI",
                    f"# ROI dimension: [x0, y0, width, height] = [{_x0}, {_y0}, {_width}, {_height}]",
                    "#",
