...
from qtpy.QtWidgets import QCheckBox, QHBoxLayout, QSizePolicy, QSpacerItem, QWidget
        ...
        for _row_index, _folder in enumerate(offset_dictionary.keys()):
            ...
            for _column_index, _text in enumerate(list_items):
                ...
                check_box.stateChanged.connect(
                    lambda state=0, row=_row_index: (
                        self.parent.visibility_checkbox_changed(state=state, row=row)
                    )
                )
