...
def extract_temperature(index=-1, temperature_array=[], time_stamp_array=[]):
    [left_T, left_index] = get_first_temperature_and_index_value(
        index=index, data_array=temperature_array, direction="left"
    )
    [right_T, right_index] = get_first_temperature_and_index_value(
        index=index, data_array=temperature_array, direction="right"
    )
...
def make_output_file_name(bin_number=-1, index=-1, algorithm="mean"):
    ...
    _output_file_name = f"bin#{bin_number:03d}_{index:04d}_{algorithm}.fits"
...
def get_working_dir(ipts="", debugging=False):
    ...
    if ipts:
        if debugging:
            _path = f"/Volumes/my_book_thunderbolt_duo/IPTS/IPTS_{ipts}"
        else:
            _path = f"/HFIR/CG1DImaging/IPTS-{ipts}/"
            print(_path)
...
def display_html_message(*, title_message="", message="", message_type="info"):
    ...
    display(
        HTML(
            f'<span style="font-size: 15px; color:{line_color}">{title_message} '
            + f'<span style="color:{message_color}"> '
            + message
            + "</span>"
        )
    )
...
class ListRunsParser:
    ...
    def make_discrete_list_of_runs(self, str_current_runs=""):
        ...
        ranges = (np.arange(int(s), int(e) + 1 if e else int(s) + 1) for s, e in spans)
        try:
    ...
    def new_runs(self, list_runs=[]):
        ...
        def match_list(reference_list=[], our_list=[]):
            _index = 0
            _ref_list_and_our_list = zip(our_list, reference_list, strict=False)
        ...
        for _group in _groups:
            if len(_group) == 2:
                ...
                if _left_value == _right_value:
                    ...
                    list_runs.append(f"{_left_value}:{_right_value}")
