...
from __code.file_handler import make_ascii_file, make_or_reset_folder
...
class ShiftTimeOffset:
    ...
    def built_list_of_fits_files(self, input_folder):
        ...
        if list_of_fits_files:
            ...
            display(
                HTML(
                    '<span style="font-size: 15px; color:green">Found '
                    + str(nbr_fits_files)
    ...
    def retrieve_name_of_timestamp_file(self, input_folder):
        ...
        if Path(timestamp_file).exists():
            display(
                HTML(
                    '<span style="font-size: 15px; color:green">Time stamp file Found ('
                    + str(timestamp_file)
                    + ")</span>"
                )
            )
    ...
    def offset_images(self):
        ...
        for _index, _current_working_folder in enumerate(list_of_folders):
            ...
            new_timestamp_filename = self.create_new_timestamp_filename(
