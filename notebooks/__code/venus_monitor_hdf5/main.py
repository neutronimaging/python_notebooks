...
from __code._utilities.file import get_full_home_file_name, make_ascii_file
...
class VenusMonitorHdf5:
    ...
    def define_settings(self):
        # bins
        bins_label = widgets.Label("Bins size (\u00b5s)")
        bins_label = widgets.Label("Bins size (100ns)")
        self.bins_ui = widgets.IntSlider(min=1, max=10000, value=100)
        bins_layout = widgets.HBox([bins_label, self.bins_ui])
        ...
        monitor_units = widgets.Label("\u00b5s")
        monitor_layout = widgets.HBox(
            [monitor_label, self.monitor_offset_ui, monitor_units],
        )

        # full layout
        full_layout = widgets.VBox([bins_layout, distance_layout, monitor_layout])
    ...
    def display_all_at_once(self):
        ...
        def plot_data(max_bin):
            ...
            ax[0].set_xlabel("TOF (\u00b5s)")
            ax[0].set_ylabel("Counts")
            ax[0].set_title("Counts vs TOF(\u00b5s)")

            ax[1].set_xlabel("Lambda (\u212b)")
            ax[1].set_ylabel("Total counts")
            ax[1].set_title("Counts vs lambda (\u212b)")
    ...
    def export_data(self):
        ...
        my_folder_selector = FileSelectorPanelWithJumpFolders(
            start_dir=start_dir,
            type="directory",
            next=self.export,
    ...
    def export(self, output_folder=None):
        for _nexus in self.list_data.keys():
            ...
            output_array = []
            for _t, _l, _y in zip(tof_axis, lambda_axis, y_axis, strict=False):
