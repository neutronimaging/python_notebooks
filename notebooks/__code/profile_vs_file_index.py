...
class ProfileVsFileIndex:
    ...
    def load_images(self):
        try:
            ...
            display(
                HTML(
                    '<span style="font-size: 20px; color:red">Please Select a set of Images First!</span>'
                )
            )
            return
    ...
    def output_profiles(self):
        ...
        output_folder = os.path.join(
            os.path.abspath(self.output_folder_ui.selected),
        ...
        for _index, _profile in enumerate(self.profile_1d):
            ...
            metadata = []
            metadata.append(f"#Image: {_file_name}")
            metadata.append(
                f"#ROI selected (y0, x0, height, width): ({roi_top}, {roi_left}, {roi_height}, {roi_width})"
            )
            ...
            metadata.append(f"#Delta Time (s): {_delta_time}")
            metadata.append("#")
            ...
            for _index_data, value in enumerate(self.profile_1d[_index]):
                data.append(f"{self.rebin_range[_index_data]}, {value}")
        ...
        display(
            HTML(
                '<span style="font-size: 20px; color:blue">Files created in folder '
                + output_folder
                + "</span>"
            )
        )
