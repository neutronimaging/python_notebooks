...
class CreateExportTimeStamp(FileFolderBrowser):
    ...
    def output_select_dsc_folder(self, folder):
        ...
        display(
            HTML(
                '<span style="font-size: 20px; color:blue">You have selected the DSC folder: '
                + folder
                + "</span>"
            )
        )
    ...
    def output_select_tiff_folder(self, folder):
        ...
        display(
            HTML(
                '<span style="font-size: 20px; color:blue">You have selected the TIFF folder: '
                + folder
                + "</span>"
            )
        )
    ...
    def run(self):
        ...
        ascii_file_name = os.path.join(
            os.path.abspath(self.output_folder), folder_name + "_timestamps.txt"
        )
        ...
        file_handler.make_ascii_file(
            data=ascii_array, output_file_name=ascii_file_name, dim="1d"
        )
