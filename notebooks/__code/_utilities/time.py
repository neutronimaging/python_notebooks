...
def get_dict_of_time_stamps(file, index_file=-1, time_zero=-1, coeff=1):
    ...
    if index_file == 0:
        ...
        delta_time_ms = delta_time * 1000.0
...
class TimestampFormatter:
    ...
    def convert_timestamp(self, timestamp):
        ...
        if o_time:
            converted_timestamp = f"{o_time.tm_year}-{o_time.tm_mon:02d}-{o_time.tm_mday:02d} {o_time.tm_hour:02d}:{o_time.tm_min:02d}:{o_time.tm_sec:02d}"
            return converted_timestamp
        else:
            raise ValueError(f"Time {timestamp} could not be converted! ")
    ...
    def get_time_dict(timestamp="", input_time_format="%m/%d/%Y %H:%M:%S"):
        ...
        try:
            ...
            ValueError(f"Error converting {timestamp} -> {input_time_format}")
            time_dict = None
