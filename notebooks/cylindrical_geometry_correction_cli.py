...
from __code.cylindrical_geometry_correction_embedded_widgets.handler import DetectionConfig, apply_cylindrical_correction, calculate_cylindrical_chord_map, compute_correction_map_factor, detect_cylindrical_boundary, replace_with_nans
...
if __name__ == "__main__":
    ...
    if not output_folder:
        logging.error("No output folder found in the config file. Please make sure the config file contains an 'output_folder' key with a valid folder path.")
        exit(1)
        
    list_of_files = config.get("list_of_images", [])
    if not list_of_files:
        logging.error("No list of images found in the config file. Please make sure the config file contains a 'list_of_images' key with a list of image file paths.")
        exit(1)
    ...
    for _file in tqdm(list_of_files):
        ...
        logging.info(f"\tApplying rotation: rotation_90_flag={rotation_90_flag}, rotation_angle={rotation_angle}")
        if rotation_90_flag:
            data = np.rot90(data)
        ...
        # apply correction
        Tcorr = apply_cylindrical_correction(T_yxl=hyperspectral_stack,
                                             C_xl=c_disc,
                                             mask_yx=res_mask,
                                             copy=True)
