...
def calculate_extrapolated_metadata(
    ...
    [left_metadata_value, left_index] = get_first_metadata_and_index_value(
        index=global_index, metadata_array=metadata_array, direction="left"
    )
    [right_metadata_value, right_index] = get_first_metadata_and_index_value(
...
new_format = f"{global_format.tm_year}/{global_format.tm_mon}/{global_format.tm_mday} {global_format.tm_hour}:{global_format.tm_min}:{global_format.tm_sec}"
print(f"{format1} -> {new_format}")
...
def format_time(old_format):
    ...
    new_format = f"{global_format.tm_year}/{global_format.tm_mon}/{global_format.tm_mday} {global_format.tm_hour}:{global_format.tm_min}:{global_format.tm_sec:02d}"
    return new_format
