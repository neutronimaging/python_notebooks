...
sys.path.insert(
    0,
    os.path.join(
        os.path.dirname(__file__), "../../__code/normalization_white_beam_at_venus"
    ),
)
...
class TestUtilities:
    def test_extract_run_number_from_file_name(self):
        ...
        returned_run_number = extract_run_number_from_file_name(
            None, file_name
        )  # Pass None for self parameter
        assert returned_run_number == expected_run_number, (
            f"Expected {expected_run_number}, but got {returned_run_number}"
        )
